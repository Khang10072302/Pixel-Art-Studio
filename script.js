(function(){
  ...
  let currentHeaderSize = 0;
  let currentCellSize = 0;

  let selection = null;      // {r0,c0,r1,c1} vùng đang chọn (chỉ số ô, bao gồm cả 2 đầu)
  let isSelecting = false;
  let selStartRow = 0, selStartCol = 0;
  ...
  grid.addEventListener('mousedown', e => {
    ...
    if(currentTool === 'select'){
      e.preventDefault();
      selectStart(e.clientX, e.clientY);
      return;
    }
    if(currentTool === 'move'){
      e.preventDefault();
  ...
  document.addEventListener('mousemove', e => {
    ...
    if(isSelecting){ selectUpdate(e.clientX, e.clientY); return; }
    if(isMoving){ moveUpdate(e.clientX, e.clientY); }
  });
  ...
  grid.addEventListener('touchstart', e => {
    const t = e.touches[0];
    if(isPanMode){ startPan(t.clientX, t.clientY); return; }
    if(isLocked) return;
    if(currentTool === 'select'){ selectStart(t.clientX, t.clientY); return; }
    if(currentTool === 'move'){ moveStart(t.clientX, t.clientY); return; }
    isPointerDown = true;
    paintCell(cellFromPoint(t.clientX, t.clientY));
  }, {passive:true});
  grid.addEventListener('touchmove', e => {
    const t = e.touches[0];
    if(isPanning){ updatePan(t.clientX, t.clientY); return; }
    if(isSelecting){ selectUpdate(t.clientX, t.clientY); return; }
    if(isMoving){ moveUpdate(t.clientX, t.clientY); return; }
  ...
  function pointToCell(clientX, clientY){
    ...
    const x = (clientX - rect.left) / scale - currentHeaderSize;
    const y = (clientY - rect.top) / scale - currentHeaderSize;
    const col = currentCellSize > 0 ? Math.floor(x / currentCellSize) : 0;
    const row = currentCellSize > 0 ? Math.floor(y / currentCellSize) : 0;
    return { row, col };
  }

  function buildSelectionShadow(row, col, sel){
    const parts = [];
    const c = 'var(--accent-mint)';
    if(row === sel.r0) parts.push(`inset 0 2px 0 0 ${c}`);
    if(row === sel.r1) parts.push(`inset 0 -2px 0 0 ${c}`);
    if(col === sel.c0) parts.push(`inset 2px 0 0 0 ${c}`);
    if(col === sel.c1) parts.push(`inset -2px 0 0 0 ${c}`);
    return parts.join(', ');
  }
  ...
  function applySelectionVisual(sel){
    for(let i = 0; i < pixelCells.length; i++){
      ...
      if(sel && row >= sel.r0 && row <= sel.r1 && col >= sel.c0 && col <= sel.c1){
        pixelCells[i].style.boxShadow = buildSelectionShadow(row, col, sel);
      } else {
        pixelCells[i].style.boxShadow = '';
      }
    }
  }
  ...
  function selectStart(clientX, clientY){
    ...
    const rc = pointToCell(clientX, clientY);
    const row = clampRow(rc.row), col = clampCol(rc.col);
    isSelecting = true;
    selStartRow = row; selStartCol = col;
    selection = { r0: row, c0: col, r1: row, c1: col };
    applySelectionVisual(selection);
  }
  function selectUpdate(clientX, clientY){
    if(!isSelecting) return;
    const rc = pointToCell(clientX, clientY);
    const row = clampRow(rc.row), col = clampCol(rc.col);
    selection = {
      r0: Math.min(selStartRow, row), r1: Math.max(selStartRow, row),
      c0: Math.min(selStartCol, col), c1: Math.max(selStartCol, col)
    };
    applySelectionVisual(selection);
  }
  function selectEnd(){
  ...
  /* ---------- di chuyển vùng đã chọn (move) ---------- */
  function applyGridColors(colorsArray){
    for(let i = 0; i < pixelCells.length; i++){
      const c = colorsArray[i] || '';
      pixelCells[i].dataset.color = c;
      pixelCells[i].style.backgroundColor = c;
    }
  }
  ...
  function moveStart(clientX, clientY){
    ...
    for(let r = 0; r < h; r++){
      const rowArr = [];
      for(let c = 0; c < w; c++){
        rowArr.push(pixelCells[(r0 + r) * cols + (c0 + c)].dataset.color);
      }
      colorsSnap.push(rowArr);
    }
    for(let r = 0; r < h; r++){
      for(let c = 0; c < w; c++){
        const idx = (r0 + r) * cols + (c0 + c);
        pixelCells[idx].dataset.color = '';
        pixelCells[idx].style.backgroundColor = '';
      }
    }
    const baseGrid = pixelCells.map(cell => cell.dataset.color);

    moveData = { w, h, colors: colorsSnap, baseGrid, origRow: r0, origCol: c0, targetRow: r0, targetCol: c0, anchorRow: rc.row, anchorCol: rc.col };
    isMoving = true;
    renderMovePreview();
  }

  function renderMovePreview(){
    const { w, h, colors, baseGrid, targetRow, targetCol } = moveData;
    const out = baseGrid.slice();
    for(let r = 0; r < h; r++){
      ...
      for(let c = 0; c < w; c++){
        ...
        if(col) out[gr * cols + gc] = col;
      }
    }
    applyGridColors(out);
    applySelectionVisual({
      r0: Math.max(0, targetRow), c0: Math.max(0, targetCol),
      r1: Math.min(rows - 1, targetRow + h - 1), c1: Math.min(cols - 1, targetCol + w - 1)
    });
  }

  function moveUpdate(clientX, clientY){
    if(!isMoving) return;
    const rc = pointToCell(clientX, clientY);
    const deltaRow = rc.row - moveData.anchorRow;
    const deltaCol = rc.col - moveData.anchorCol;
    moveData.targetRow = Math.max(0, Math.min(rows - moveData.h, moveData.origRow + deltaRow));
    moveData.targetCol = Math.max(0, Math.min(cols - moveData.w, moveData.origCol + deltaCol));
    renderMovePreview();
  }
  ...
  function moveEnd(){
    ...
    isMoving = false;
    document.body.style.userSelect = '';
    const { w, h, targetRow, targetCol } = moveData;
    selection = { r0: targetRow, c0: targetCol, r1: targetRow + h - 1, c1: targetCol + w - 1 };
    moveData = null;
    pushHistory();
  }
  ...
  /* ---------- zoom & pan canvas ---------- */
  function clampPan(){
    const scaledW = grid.offsetWidth * zoomLevel;
    const scaledH = grid.offsetHeight * zoomLevel;
    const maxX = Math.max(0, (scaledW - canvasWrap.clientWidth) / 2);
    const maxY = Math.max(0, (scaledH - canvasWrap.clientHeight) / 2);
  ...
  function endPan(){
    if(!isPanning) return;
    isPanning = false;
    canvasWrap.style.cursor = isPanMode ? 'grab' : '';
    document.body.style.userSelect = '';
  }

  panToggle.addEventListener('click', () => {
    isPanMode = !isPanMode;
    panToggle.classList.toggle('active', isPanMode);
    canvasWrap.style.cursor = isPanMode ? 'grab' : '';
  });
